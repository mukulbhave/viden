...
def get_boxes_for_id(voc_path, year, image_id):
    ...
    boxes : array of int
        ...
        5xN array.
    """
    fname = os.path.join(voc_path, '{}/{}.xml'.format(year,
                                                                     image_id))
...
def get_image_for_id(voc_path, year, image_id):
    ...
    """
    fname = os.path.join(voc_path, '{}/{}.jpg'.format(year,image_id))
    with open(fname, 'rb') as in_file:
      data = in_file.read()
    # Use of encoding based on: https://github.com/h5py/h5py/issues/745
    print("Image :"+fname)
    #x = np.asarray(Image.open(fname))
...
def get_ids(voc_path, datasets):
    ...
    for year, image_set in datasets:
        id_file = os.path.join(voc_path, '{}/{}.txt'.format(
            year, image_set))
